      ...
      <header className="sticky top-0 z-50 bg-cream border-b border-kraft-light">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-4 py-3 sm:py-4">
            ...
            <button
              onClick={openCart}
              className="cursor-pointer inline-flex items-center gap-2 bg-cream-dark border border-kraft-light text-ink hover:border-kraft hover:text-kraft transition-colors duration-150 font-body text-sm font-medium px-4 py-2.5 rounded focus:outline-none focus:ring-2 focus:ring-kraft"
              aria-label={`Shopping basket, ${totalCount} ${totalCount === 1 ? 'item' : 'items'}`}
            >
              <svg
              ...
              <span>Basket</span>
              {totalCount > 0 && (
                <span className="bg-kraft text-cream text-xs rounded-full h-5 w-5 flex items-center justify-center font-body font-semibold leading-none tabular-nums">
                  {totalCount}
                </span>
              )}
            </button>
